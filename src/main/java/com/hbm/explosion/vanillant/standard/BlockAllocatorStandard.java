					...
					if(i == 0 || i == this.resolution - 1 || j == 0 || j == this.resolution - 1 || k == 0 || k == this.resolution - 1) {
						...
						for(float stepSize = 0.3F; powerRemaining > 0.0F; powerRemaining -= stepSize * 0.75F) {
							...
							if(state.getMaterial() != Material.AIR) {
								float blockResistance = explosion.exploder != null ? explosion.exploder.getExplosionResistance(explosion.compat, world, new BlockPos(blockX, blockY,blockZ), state) :
										block.getExplosionResistance(null);
								powerRemaining -= (blockResistance + 0.3F) * stepSize;

							}

							if(powerRemaining > 0.0F && (explosion.exploder == null || explosion.exploder.getExplosionResistance(explosion.compat, world, new BlockPos(blockX, blockY,blockZ), state)<powerRemaining)) {
